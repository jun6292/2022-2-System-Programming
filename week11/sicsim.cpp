    ...
    while (true) {
        ...
        if (input == 'r') { // 'r'을 입력했을 때 처리
            for (int j = i; j < i + run; j++) {
                string code = objcode_vec[j].substr(0, 2);  // 오브젝트 코드의 앞 두 자리: 연상 연산 코드
                string s_addr = objcode_vec[j].substr(2, 4);  // 오브젝트 코드의 뒤 네 자리: operand의 주소
                unsigned long long ull_addr = strtoull_addr(s_addr);
                stringstream ss(line_vec[j]), fm(file_map[ull_addr]);
                ss.str(line_vec[j]); fm.str(file_map[ull_addr]);
                // label, opcode, operand 공백 기준으로 자른다.
                ...
                if (s_opcode == "word" || s_opcode == "byte" || s_opcode == "resb" || s_opcode == "resw")
                    break;  // 선언문이면 종료
                cout << objcode_vec[j] << ' ' << s_opcode << ' ' << s_operand << ' ' << '\n';
